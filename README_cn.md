...
`meteor-image` 是一个 Codex 技能，用于通过 `meteor041.com` 这一 OpenAI-compatible 代理生成图片。

它可以无缝配合 OpenAI-compatible 代理使用，例如 meteor041.com，并直接复用你当前在 Codex 中已经配置好的代理设置来完成图片生成。
...
如果你的 Codex 已经配置为使用 meteor041.com，那么图片生成可以直接开箱即用，不需要额外配置。

这意味着如果 Codex 已经配置好了 `meteor041.com` 代理，这个技能可以直接复用同一套设置，而不需要你每次手动传入单独的 base URL 或 API key。
...
内置的 `imagegen` 技能主要面向 Codex 默认内建的图片生成流程。这个自定义技能用于你明确希望通过第三方 OpenAI-compatible 代理来路由图片生成，并复用本地磁盘上已有 Codex 代理配置的场景。

## 说明

- 当前代理可用于 `/images/generations`
